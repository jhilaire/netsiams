...
p_emiAll_bySec_byTemp <- ggplot(tmp %>% ) + 
  ...
  geom_segment(aes(x=xmin, xend=xmax, y=ymin, yend=ymax), data=data.frame(variable=c("CO2 - Power", "CO2 - Industry", 
                                                                                     "CO2 - Buildings","CO2 - Transportation", "CO2 - Land-Use",
                                                                                     "nonCO2KG"), xmin=0.5, xmax=3.5, ymin=0, ymax=0), lty=2, color="red") +
